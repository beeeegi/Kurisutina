...
async def leaderboard(interaction):
    ...
    print(f"chamonatvali: {enumerate(top_users, start=1)}")
...
class MyView(discord.ui.View):
    ...
    async def button_callback(self, button, interaction):
        await interaction.response.edit_message("xle oxond meore")

@tree.command(name="sex", description="sex?", guild=server)
async def sex(interaction):
    view = MyView()
    await interaction.response.send_message("xle", view=view)
